...
class NyanChan {
        ...
        waifu.innerHTML = `
            <div class="waifu-container">
                <div class="waifu-character">
                    <div class="waifu-face">${this.expressions[this.currentExpression]}</div>
                    <div class="waifu-body">🐱</div>
    ...
    createToolbar() {
        ...
        toolbar.innerHTML = `
            <button class="toolbar-btn" title="主页" onclick="nyanChan.goHome()">🏠</button>
            <button class="toolbar-btn" title="对话" onclick="nyanChan.startConversation()">💬</button>
            <button class="toolbar-btn" title="推荐游戏" onclick="nyanChan.recommendGame()">🎮</button>
            <button class="toolbar-btn" title="换表情" onclick="nyanChan.changeExpression()">🎭</button>
            <button class="toolbar-btn" title="夜间模式" onclick="nyanChan.toggleNightMode()">🌙</button>
            <button class="toolbar-btn" title="背景音乐" onclick="nyanChan.toggleMusic()">🎵</button>
            <button class="toolbar-btn" title="隐藏" onclick="nyanChan.hide()">❌</button>
        `;
        
        document.querySelector("#nyan-chan .waifu-container").appendChild(toolbar);
    }
    ...
    createMusicPlayer() {
        ...
        "RE4KTK7h8bllHgg2jdXzzn0vBSF+zO/ahjMGHm7A7+OZURE";
    ...
    showMessage(text, duration = 3000) {
        const messageEl = document.querySelector("#nyan-chan .message-text");
        if (!messageEl) return;
    ...
    changeExpression() {
        ...
        this.currentExpression = expressions[nextIndex];
        const faceEl = document.querySelector("#nyan-chan .waifu-face");
        if (faceEl) {
            faceEl.textContent = this.expressions[this.currentExpression];
        }
        
        this.showMessage("喵~ 我换了新表情呢！喜欢吗？", 2000);
    }
    ...
    toggleMusic() {
        ...
        if (this.musicPlaying) {
            ...
            audio.play().catch(() => {
                this.showMessage("喵~ 音乐播放失败了，可能需要用户交互才能播放呢！", 3000);
            });
    ...
    hide() {
        const waifu = document.getElementById("nyan-chan");
        if (waifu) {
